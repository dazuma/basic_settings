...
module TestFixtures
  class Settings1 < BasicSettings
    class FooSettings < BasicSettings
      class BarSettings < BasicSettings
        settings_attr(:baz)
      end
    end

    class QuxSettings < BasicSettings
      settings_attr(:ho)
    end
  end
...
describe BasicSettings do
  ...
  describe "attribute" do
    ...
    describe "type specification" do
      ...
      it "matches boolean defaults" do
        ...
        settings.foo = false
        assert_same(false, settings.foo)
        err = assert_raises(BasicSettings::FieldError) do
          settings.foo = nil
        end
        assert_equal(
      ...
      it "recognizes a block type spec" do
        settings_class.settings_attr(:foo, default: 0) do |val|
          val >= 0 ? val : BasicSettings::ILLEGAL_VALUE
        end
        assert_equal(0, settings.foo)
        settings.foo = 0.1
    ...
    describe "parentage" do
      let(:child_class) { Class.new(BasicSettings) }
  ...
  describe "auto-hierarchy" do
    it "builds the hierarchy based on nesting" do
      settings = TestFixtures::Settings1.new
  ...
  describe "subclass of a settings class" do
    it "inherits the fields" do
      settings = TestFixtures::Settings3.new
      settings.haha = "hello"
      settings.hehe = "bye"
  ...
  describe "data loading" do
    ...
    it "loads a YAML file" do
      settings_class.settings_attr(:foo)
      settings_class.settings_attr(:bar)
      errors = settings.load_yaml_file!(File.join(data_dir, "input.yaml"))
      assert_empty(errors)
      assert_equal("hello", settings.foo)
      assert_nil(settings.bar)
    end
    ...
    it "loads a JSON file" do
      settings_class.settings_attr(:foo)
      settings_class.settings_attr(:bar)
      errors = settings.load_json_file!(File.join(data_dir, "input.json"))
      assert_empty(errors)
      assert_equal("hello", settings.foo)
      assert_nil(settings.bar)
    end
